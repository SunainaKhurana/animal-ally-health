...
import { ChevronDown, ChevronUp, AlertTriangle, CheckCircle, Clock, Trash2, Sparkles, Edit2, Save, X, ExternalLink, Link } from "lucide-react";
...
const HealthReportCard = ({ report, onDelete, autoExpand = false }: HealthReportCardProps) => {
  ...
  return (
    <Card className={`${report.status === 'completed' ? 'border-green-200 bg-green-50/30' : ''} shadow-sm hover:shadow-md transition-shadow`}>
      <CardHeader>
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <CardDescription>
              {report.report_type} • {new Date(report.actual_report_date || report.report_date).toLocaleDateString()}
            </CardDescription>
          </div>
